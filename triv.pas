...
This program may be freely modified and/or distributed provided:
 ...
 5. No one tells the UPEI computer center about this.
I apologize for creating this waste of time and effort and, even worse, for not
documenting it.  I am not responsible nor liable for losses (real or imagined)
incurred due to the use, misuse, abuse, and/or overuse of this program.  I make
no apologies for the fact this program uses external procedures. -- JWL}
...
type    grid=array[1..N,1..N]of integer;
...
begin
	...
	if i=509 then begin
		writeln('We don''t have all day.'); i:=0
	end;
	readletter:=i
end;
...
begin
	...
	if(randb(100)<=m2hit[monster[x,y]]-20)then begin
		writeln('tears at the webs holding it.');
		hpweb:=hpweb-randb(mdmg[monster[x,y]]);
		if(hpweb<=0)then writeln('The ',creature[monster[x,y]],
			' broke free!');
...
case command of
	99{c}:if(spellsknown=0)then writeln('You know no spells to cast.')
	else begin
		...
		if(j<1)or(j>spellsknown)then
			...
			writeln('You are too weak to cast ',
				spellname[spellgot[j]],'.')
		else begin
			spell:=spellgot[j];
	...
	102{f}:begin
		...
		then begin
			...
			mhp[x,y]:=mhp[x,y]-randb(weapdmg[gotweapon])
				-(xplev div 6);
		end else begin
			write('You '); missed;
			writeln(' the ',creature[monster[x,y]],'.')
	...
	63{?}:begin
		...
		writeln('  <=up, >=down');
		writeln(' spells: c=cast spell, l=learn scroll, r=read ',
			'scroll');
		writeln(' potions: d=drink, t=throw');
		writeln(' miscellaneous: q=quit, v=version, x=redraw screen, ',
			'?=help, m=map symbols,');
		writeln('  o=output saved game, i=input saved game, ',
			'e=eat food,');
	...
	108{l}:if(scrkinds=0)then writeln('You have no scrolls to learn from.')
		...
		if(j<1)or(j>scrkinds)then
			...
			if found then writeln('You already know it.')
			else begin
	...
	100{d}:if(potkinds=0)then writeln('You have no potions to drink.')
	else begin
		writeln('You have:');
		listpotions;
	...
	114{r}:if(scrkinds=0)then writeln('You have no scrolls to read.')
	else begin
		writeln('You have:');
		listscrolls;
	...
	36{$}:if rooms[x,y]=vendor then runvend else
...
if(spell>0)then case spell of
	...
	4:begin
		...
		dx:=0; dy:=0; hpweb:=0;
		repeat dx:=randb(N)-x; dy:=randb(N)-y
		until(dx<>0)and(dy<>0);
		drawroom(dx,dy);
		if(monster[x,y]>0)or(rooms[x,y]=vendor)then begin
			writeln('However, it seems the room ',
				'is already occupied.');
			warn;
		end
	end;
	...
	8:if rooms[x,y]=ladder then
		...
		if monster[x,y]>0 then begin
			writeln('The ',creature[monster[x,y]],' appears ',
				'insulted at it is gored by the ladder.');
			mhp[x,y]:=0
		end;
	end;
	...
	if monster[x,y]>0 then begin
		...
		case throw of
			...
			3:begin
				...
				if mhp[x,y]<=0 then writeln('It collapses and ',
					'dies, a victim of contaminated ',
					'potions.');
				found:=true;
			end;
			5:begin
			...
			otherwise begin
				writeln('The ',creature[monster[x,y]],' is ',
					'slightly hurt by the glass shards.');
				mhp[x,y]:=mhp[x,y]-1;
			end;
		end;
		...
		repeat
			...
			if hunger>=300 then begin
				...
				if gotarmor>1 then begin
					writeln('When you wake, you find your ',
						'armor is gone.');
					gotarmor:=1;
					locate(8,N*3+33);
					write(armorname[gotarmor],esc,'[K')
				end else begin
					writeln('You suffered some damage ',
						'from a concussion.');
